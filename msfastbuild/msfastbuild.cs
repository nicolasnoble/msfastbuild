...
namespace msfastbuild
{
    public class Options
    {
        ...
        [Option('p', "parallel", DefaultValue = false,
        HelpText = "Parallelize the project builds.")]
    ...
    public class msfastbuild
    {
        ...
        public class MSFBProject
        {
            ...
            public bool HasCompileActions = false;
            public System.Diagnostics.Process BuildProcess = null;
            public string BFFOutputFilePath = "";
            public bool HasBuilt()
            {
                if (BuildProcess == null) return false;
                return BuildProcess.HasExited;
            }
        }

        public class ProcessWaitHandle : System.Threading.WaitHandle
        {
            public System.Diagnostics.Process Process = null;
            public ProcessWaitHandle(System.Diagnostics.Process process) {
        ...
        static void Main(string[] args)
        {
            ...
            {
                string VCTargetsPath = project.Proj.GetPropertyValue("VCTargetsPath");
                if (string.IsNullOrEmpty(VCTargetsPath))
                {
                    VCTargetsPath = project.Proj.GetPropertyValue("VCTargetsPathActual");
                }
                if (string.IsNullOrEmpty(VCTargetsPath))
                {
                    Console.WriteLine("Failed to evaluate VCTargetsPath variable on " + Path.GetFileName(project.Proj.FullPath) + ". Is this a supported version of Visual Studio?");
                    continue;
                }
                ...
                project.BFFOutputFilePath = Path.GetDirectoryName(project.Proj.FullPath) + "\\" + Path.GetFileName(project.Proj.FullPath) + "_" + CommandLineOptions.Config.Replace(" ", "") + "_" + CommandLineOptions.Platform.Replace(" ", "") + ".bff";
                GenerateBffFromVcxproj(project, CommandLineOptions.Config, CommandLineOptions.Platform);
            }

            if (CommandLineOptions.GenerateOnly)
            {
                Console.WriteLine("Generated " + EvaluatedProjects.Count + " projects.");
                return;
            }

            int ProjectsBuilt = 0;
            int ErrorCount = 0;
            if (CommandLineOptions.Parallel)
            {
                List<ProcessWaitHandle> processes = new List<ProcessWaitHandle>();
                while ((ProjectsBuilt != EvaluatedProjects.Count) && ((ErrorCount == 0) || (!CommandLineOptions.BreakEarly)))
                {
                    List<MSFBProject> toBuildThisRound = new List<MSFBProject>();
                    foreach (MSFBProject project in EvaluatedProjects)
                    {
                        if ((processes.Count + toBuildThisRound.Count) >= 64) break;
                        if (project.BuildProcess != null) continue;
                        bool hasPendingDependencies = false;
                        foreach (MSFBProject d in project.Dependencies)
                        {
                            if (!d.HasBuilt())
                            {
                        ...
                        toBuildThisRound.Add(project);
                    }
                    foreach (MSFBProject project in toBuildThisRound)
                    {
                        var process = ExecuteBffFile(project.BFFOutputFilePath, project.Proj.FullPath, CommandLineOptions.Platform);
                        project.BuildProcess = process;
                        processes.Add(new ProcessWaitHandle(process));
                    }
                    System.Threading.WaitHandle.WaitAny(processes.ToArray());
                    ...
                    processes.RemoveAll(p => p.Process.HasExited);
                }
            }
            else
            {
                foreach (MSFBProject project in EvaluatedProjects)
                {
                    if (project.HasCompileActions)
                    {
                        var process = ExecuteBffFile(project.BFFOutputFilePath, project.Proj.FullPath, CommandLineOptions.Platform);
                        process.WaitForExit();
                        if (process.ExitCode != 0)
                        {
                            if (CommandLineOptions.BreakEarly) break;
                        }
                    }
                    ProjectsBuilt++;
                }
                ...
                try
                {
                    ...
                    if (newProj != null)
                    {
                        ...
                        if (dependent != null)
                        {
                            ...
                            dependent.Dependencies.Add(newProj);
                        }
                    }
                    else
                    {
        ...
        static public bool HasFileChanged(string BFFOutputFilePath, string InputFile, string Platform, string Config, out string MD5hash)
        {
        ...
        static public System.Diagnostics.Process ExecuteBffFile(string BFFOutputFilePath, string ProjectPath, string Platform)
        {
        ...
        static private void GenerateBffFromVcxproj(MSFBProject CurrentProject, string Config, string Platform)
        {
            Project ActiveProject = CurrentProject.Proj;
            string MD5hash = "wafflepalooza";
            PreBuildBatchFile = "";
            PostBuildBatchFile = "";
            bool FileChanged = HasFileChanged(CurrentProject.BFFOutputFilePath, ActiveProject.FullPath, Platform, Config, out MD5hash);
            ...
            string Suffix = (Platform == "Win64" || Platform == "x64") ? "x64" : "x86";
            ...
            string ExecutablePathProperty = string.Format("VC_ExecutablePath_x64_{0}", Suffix);
            string CompilerRoot = ActiveProject.GetProperty(ExecutablePathProperty).EvaluatedValue;
            OutputString.AppendFormat(".CompilerRoot = '{0}'\n", CompilerRoot);

            WindowsSDKTarget = ActiveProject.GetProperty("TargetPlatformVersion").EvaluatedValue;
            OutputString.AppendFormat(".TargetPlatformVersion = '{0}'\n", WindowsSDKTarget);
            ...
            CompilerString.Append("\t.Executable = '$CompilerRoot$/cl.exe'\n");
            CompilerString.Append("\t.ExtraFiles =\n\t{\n");
            foreach (var f in Directory.GetFiles(CompilerRoot, "*.*", SearchOption.AllDirectories))
            {
                string name = f.Substring(CompilerRoot.Length);
                if (!name.EndsWith("cl.exe"))
                    CompilerString.AppendFormat("\t\t'$CompilerRoot${0}'\n", name);
            }
            //			foreach (var f in Directory.GetFiles(ActiveProject.GetProperty("VCToolsRedistInstallDir").EvaluatedValue, "*.*", SearchOption.AllDirectories))
            //			{
            //				CompilerString.AppendFormat("\t\t'{0}'\n", f);
            //			}
            CompilerString.Append("\t}\n"); //End extra files
            CompilerString.Append("}\n\n"); //End compiler

            CompilerString.Append("Compiler('rc')\n{\n");
            CompilerString.Append("\t.Executable = '$WindowsSDKBasePath$\\bin\\$TargetPlatformVersion$\\x64\\rc.exe'\n");
            ...
            if (ActionNumber > 0)
            {
                CurrentProject.HasCompileActions = true;
            }
            else
            {
                CurrentProject.HasCompileActions = false;
            ...
            if (BuildOutput == BuildType.Application || BuildOutput == BuildType.DynamicLib)
            {
                ...
                OutputString.Append("\t.Linker = '$CompilerRoot$\\link.exe'\n");
                ...
                if (CurrentProject.HasCompileActions)
                {
            ...
            else if (BuildOutput == BuildType.StaticLib)
            {
                ...
                OutputString.Append("\t.Librarian = '$CompilerRoot$\\lib.exe'\n");
            ...
            {
                File.WriteAllText(CurrentProject.BFFOutputFilePath, OutputString.ToString());
            }
        }
